package com.example.signin2
    ...
    class LoginViewModel : ViewModel() {
        ...
        //get-user-profile
        private var _profileSuccess = MutableStateFlow<Boolean?>(false)
        val profileSuccess : StateFlow<Boolean?> = _profileSuccess

        private var _profileResponse = MutableStateFlow<ProfileResponse?>(null)
            ...
            viewModelScope.launch {
                try {
                    ...
                    if (response.isSuccessful) {
                        ...
                        Log.d("login response", "Response body: ${response.body()}")

                        // Print sign-in data to logs
                        Log.d("login data", "Sign-in data: $data")
                        ...
                        Log.e("login failure", "Unsuccessful response: ${response.code()}")
            ...
            viewModelScope.launch {
                try {
                    ...
                    if (response.isSuccessful) {
                        ...
                        // Print response body to logs
                        Log.d("register response", "Response body: ${response.body()}")

                        // Print sign-in data to logs
                        Log.d("register data", "Sign-in data: $data1")

                        // Perform further actions with the response data if needed
                        // For example, update UI or navigate to another screen
                    } else {
                        // Handle unsuccessful response (e.g., server error, invalid credentials)
                        Log.e("register failure", "Unsuccessful response: ${response.code()}")
                        _signupresponse.value = response.body()
        ...
        fun tokennn(token : String){
            viewModelScope.launch {
                ...
                if(response.isSuccessful){
                    ...
                    // Print response body to logs
                    Log.d("Token response", "Response body: ${response.body()}")

                    // Print Token data to logs
                    Log.d("Tokken", "Sign-in data: $token")
                }
                else {
                    _tokenResponse.value = false
                    Log.d("Token failure", "Response body: ${response.body()}")
        ...
        fun fetchUserProfile(token: String){
            viewModelScope.launch {
                ...
                try {
                    ...
                    if (response.isSuccessful) {
                        Log.d("User profile response", "${response.body()}")

                        _profileResponse.value = response.body()
                        _profileSuccess.value = true
                    }
                    else {
                        Log.d("get user profile error","${response.body()}")

                        _profileResponse.value = response.body()
                        _profileSuccess.value = false
                    }
                }
                catch(e : Exception){
